...
public class Spanish extends AppCompatActivity {
    ...
    final public static int[][] keyboard_id = {
            ...
            {R.id.Enter, R.id.C, R.id.V, R.id.B, R.id.N, R.id.M, R.id.specialN, R.id.accents, R.id.Delete},
    };

    final public static int[][] keyboard_color = new int[4][9];
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (input.charAt(i) == chosenWord.charAt(i)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    if (keyPosition == -1) keyPosition = keyboard_position.indexOf(addAccent(input.charAt(i)));
    ...
    public static Intent makeIntent(Context context) {
        return new Intent(context, Spanish.class);
    }

    private void setupEndActivityButton() {
        Button btn = findViewById(R.id.spanish_back_button);
    ...
    /* This section contains all the containers that are needed in most methods above.
       ...
       addSpanishAccents() converts those words back to their original forms*/

    public static String[] triedWords = {"", "", "", "", "", "", ""};
    public String chosenWord;
    private static Vector<String> wordList = new Vector<>();

    private static String addSpanishAccents(String fromTxtFile) {
        char[] letters = {'á', 'é', 'í', 'ó', 'ú', 'ü', 'ñ'};
        for (int i = 1; i <= 7; i++) {
            fromTxtFile = fromTxtFile.replace((char)('0' + i), letters[i-1]);
        }
        return fromTxtFile;
    }
    ...
    private void loadFiles() {
        String line;
        try {
            InputStream inputStream = getResources().openRawResource(R.raw.spanish_words);

            BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream));
            while ((line = reader.readLine()) != null) {
                wordList.add(addSpanishAccents(line));
            }
            inputStream.close();
        }
        catch (IOException e) {
